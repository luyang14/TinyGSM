...
#ifndef SRC_TINYGSMCLIENTEC20X_H_
#define SRC_TINYGSMCLIENTEC20X_H_
// #pragma message("TinyGSM:  TinyGsmClientEC20X")
...
class TinyGsmEC20X : public TinyGsmModem<TinyGsmEC20X>,
                    public TinyGsmGPRS<TinyGsmEC20X>,
                    public TinyGsmTCP<TinyGsmEC20X, TINY_GSM_MUX_COUNT>,
                    public TinyGsmCalling<TinyGsmEC20X>,
                    public TinyGsmSMS<TinyGsmEC20X>,
                    public TinyGsmTime<TinyGsmEC20X>,
                    public TinyGsmNTP<TinyGsmEC20X>,
                    public TinyGsmGPS<TinyGsmEC20X>,
                    public TinyGsmBattery<TinyGsmEC20X>,
                    public TinyGsmTemperature<TinyGsmEC20X> {
  friend class TinyGsmModem<TinyGsmEC20X>;
  friend class TinyGsmGPRS<TinyGsmEC20X>;
  friend class TinyGsmTCP<TinyGsmEC20X, TINY_GSM_MUX_COUNT>;
  friend class TinyGsmCalling<TinyGsmEC20X>;
  friend class TinyGsmSMS<TinyGsmEC20X>;
  friend class TinyGsmTime<TinyGsmEC20X>;
  friend class TinyGsmNTP<TinyGsmEC20X>;
  friend class TinyGsmGPS<TinyGsmEC20X>;
  friend class TinyGsmBattery<TinyGsmEC20X>;
  friend class TinyGsmTemperature<TinyGsmEC20X>;

  /*
   * Inner Client
   */
 public:
  class GsmClientEC20X : public GsmClient {
    friend class TinyGsmEC20X;

   public:
    GsmClientEC20X() {}

    explicit GsmClientEC20X(TinyGsmEC20X& modem, uint8_t mux = 0) {
      init(&modem, mux);
    }

    bool init(TinyGsmEC20X* modem, uint8_t mux = 0) {
      this->at       = modem;
      sock_available = 0;
      prev_check     = 0;
  ...
  /*
  class GsmClientSecureEC20X : public GsmClientEC20X
  {
  public:
    GsmClientSecure() {}

    GsmClientSecure(TinyGsmEC20X& modem, uint8_t mux = 0)
  ...
  */
   ...
   */
 public:
  explicit TinyGsmEC20X(Stream& stream) : stream(stream) {
 ...
 protected:
  bool initImpl(const char* pin = NULL) {
    ...
    DBG(GF("### TinyGSM Compiled Module:  TinyGsmClientEC20X"));
 ...
 protected:
  bool gprsConnectImpl(const char* apn, const char* user = NULL,
    ...
    //sendAT(GF("+CGDCONT=1,1,\""), apn, GF("\",\""), user, GF("\",\""), pwd,
    //       GF("\""));
    //if (waitResponse() != 1) { return false; }
 ...
 protected:
  ...
  // The EC20X returns UTC time instead of local time as other modules do in
 ...
 protected:
  GsmClientEC20X* sockets[TINY_GSM_MUX_COUNT];
  const char*    gsmNL = GSM_NL;
};

#endif  // SRC_TINYGSMCLIENTEC20X_H_
